(function(define) {
    define([], function() {
        ...
        function _shimNodes(nodes, obj) {
            if (!nodes) return;
            var i = 0;
            if (nodes.nodeType) return obj[obj.length++] = nodes;
        ...
        function Core(toSelect, what) {
            ...
            if (!toSelect) {
            ...
            } else if ($.isArray(toSelect) || toSelect.length != undefined && !$.isString(toSelect)) {
                ...
                if (toSelect.length == undefined) {
                    ...
                    for (var i = 0; i < toSelect.length; i++) if (toSelect[i].parentNode == what) this[this.length++] = toSelect[i];
                }
                return this;
            } else if ($.isObject(toSelect) && what == undefined) {
                //Single object
                if (toSelect.nodeType) this[this.length++] = toSelect;
                return this;
        ...
        var each = $.each = function(obj, iterator, context) {
            ...
            if (obj.length === +obj.length) {
                if (context) for (var i = 0, l = obj.length; i < l; i++) {
                    ...
                    if (iterator.call(obj[i], obj[i], i, obj) === false) return;
                }
            } else {
                if (context) for (var key in obj) {
                    ...
                    if (obj.hasOwnProperty(key)) {
                        if (iterator.call(obj[key], obj[key], i, obj) === false) return;
                    }
                }
            }
        };
        ...
        $.is$ = function(obj) {
            return obj instanceof $.fn.constructor;
        };
        ...
        $.extend = function(target) {
            ...
            if (arguments.length === 1 && $.isObject(target)) {
                for (key in target) {
                    ukey = key.split(":");
                    if (ukey.length == 1) $.fn[key] = target[key]; else if (ukey[0] == "util") _utilCache[ukey[1]] = target[key];
                }
                return;
            } else {
                var a = slice.call(arguments, 1);
                for (var i = 0, len = a.length; i < len; i++) for (key in a[i]) target[key] = a[i][key];
            }
            return target;
        };
        ...
        $.get = function(url) {
            ...
            for (var i = 1, len = arguments.length; i < len; i++) {
                ...
                switch (typeof t) {
                  ...
                  case "function":
                    if (success === empty) success = t; else error = t;
                    break;
                }
            }
            return ajax({
	 ...
	 */
        $.queryString = function(obj, prefix) {
            ...
            if (obj instanceof $.fn.constructor) {
                for (var i = 0, len = obj.length; i < len; i++) {
                    var k = prefix ? prefix + "[]" : obj[i].name, v = obj[i].value;
        ...
        function ajax(opts) {
            ...
            try {
                ...
                if ($.isObject(settings.data)) settings.data = $.queryString(settings.data);
        ...
        function errCallback(opts) {
            if ($.DEBUG.open) {
                if (!opts.orgError) opts.orgError = opts.error; else return opts.error;
                return function(e) {
        ...
        function _insertFragments(jqm, container, insert) {
            ...
            if (insert) {
                ...
                for (var j = 0; j < jqm.length; j++) frag.appendChild(jqm[j]);
        ...
        $.extend({
            html: function(html, init) {
                ...
                if (html === undefined) return this[0].innerHTML.replace(/_id\=\"[\w\,]*?\"/gi, "");
            ...
            beforeTo: function(target, init, after) {
                if (this.length == 0) return this;
                var targets = $(target);
                if (!targets || !targets.parent().length) return this;
            ...
            before: function(content, init) {
                var obj = $(content);
                if ((!obj || !obj.length) && typeof content == "string") obj = $(document.createTextNode(content));
        ...
        function shim_id(element) {
            ...
            if (id && (!_attrCache[id] || !keys(_attrCache[id]).length) && (!_propCache[id] || !keys(_propCache[id]).length) && (!_initedCache[id] || !_initedCache[id].length) && (!handlers[id] || !handlers[id].length)) element.removeAttribute("_id");
        }
        $.extend({
            ...
            removeAttr: function(attr) {
                ...
                for (var i = 0, len = this.length; i < len; i++) {
                    ...
                    for (var j = 0, lem = attrs.length; j < lem; j++) {
                        ...
                        if (id && _attrCache[id] && _attrCache[id][at]) delete _attrCache[id][at];
                    }
                    shim_id(el);
                }
                return this;
        ...
        $.extend({
            ...
            filter: function(selector) {
                ...
                for (var i = 0, len = this.length; i < len; i++) {
                    var val = this[i];
                    if (val.parentNode && $(selector, val.parentNode).index(val) >= 0) elems.push(val);
                }
        ...
        $.extend({
            ...
            offset: function() {
                ...
                }; else var obj = this[0].getBoundingClientRect();
                return {
        ...
        function eachEvent(events, fn, iterator) {
            if ($.isObject(events)) $.each(events, iterator); else {
        ...
        function add(element, events, fn, selector, getDelegate) {
            var id = _id(element), set = handlers[id] || (handlers[id] = []);
